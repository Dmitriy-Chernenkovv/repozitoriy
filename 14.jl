...
function pravoverh(r, side1, side2)
    p = ""
    while !(isborder(r, side1) & isborder(r, side2))
        if ! isborder(r, side1)
            move!(r, side1)
...
function cherezstenu(r::Robot, side::HorizonSide, steps::Int, steps2::Int)
    ort = naz(side)
    if trymove!(r, side)
        steps2 += 1
        if !isborder(r, inverse(ort))
            ...
            steps, steps2 = cherezstenu(r, side, steps, steps2)
        end
        return (steps, steps2)
    else
        if isborder(r, side)
            move!(r, ort)
            x, y = cherezstenu(r, side, steps + 1, steps2)
            steps = x
            steps2 = y
            return (steps, steps2)
        end
